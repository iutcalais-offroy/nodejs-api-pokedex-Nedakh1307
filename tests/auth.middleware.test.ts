...
describe('Deck Controller', () => {
  ...
  describe('create', () => {
    ...
    it('should return 400 if cards count is not 10', async () => {
      const req = { body: { name: 'Fail', cards: [1, 2, 3] }, user: { userId: 1 } } as any;
      const res = makeRes();
      await deckController.create(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
  ...
  describe('update', () => {
    it('should update name only (no cards)', async () => {
      ...
      prismaMock.$transaction.mockResolvedValue({ id: 1, name: 'NewName', userId: 1, cards: [] } as any);
      const req = {
        params: { id: '1' },
        body: { name: 'NewName' },
        user: { userId: 1 }
    ...
    it('should update with cards successfully', async () => {
      prismaMock.deck.findFirst.mockResolvedValue({ id: 1, userId: 1, name: 'Old' } as any);
      prismaMock.card.findMany.mockResolvedValue(new Array(10).fill({ id: 1 }) as any);
      prismaMock.$transaction.mockResolvedValue({ id: 1, name: 'Updated', userId: 1, cards: [] } as any);
      const req = {
        params: { id: '1' },
